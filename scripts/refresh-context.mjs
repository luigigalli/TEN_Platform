...
class ContextRefresher {
  constructor(projectRoot) {
    ...
    this.apiEndpoint = process.env.WINDSURF_API_ENDPOINT || 'https://windsurf.codeium.com/api/v1';
    this.apiKey = process.env.WINDSURF_API_KEY;
    
    if (!this.apiKey) {
      throw new Error('WINDSURF_API_KEY is required but not set');
    }
  }
  ...
  async sendContextToAssistant(conversationId, context) {
    try {
      const response = await fetch(`${this.apiEndpoint}/conversations/${conversationId}/context`, {
        method: 'POST',
        headers: {
